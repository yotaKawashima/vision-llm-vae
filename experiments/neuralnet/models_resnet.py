...
class Encoder(BaseModel):
    ...
    def latent_alignment_loss(
        ...
        if llm_alignment_loss_type == "norm_and_cosine_similarity":
            ...
            soft_loss = self.soft_nn_loss(
                latent_variables, text_emb, temperature=temperature
            )
            norms = latent_variables.norm(p=2, dim=1)
            norm_loss = F.mse_loss(norms, torch.ones_like(norms), reduction="mean")
            return soft_loss, norm_loss
    ...
    def compute_loss(self, **kwargs) -> dict:
        ...
        **kwargs:
            ...
            temperature: float
                Temperature for soft_nn loss (required for "soft_nn" and "norm_and_soft_nn").
        Returns
        -------
        dict
            Computed loss
...
class BetaVAEScalingLLM(BetaVAE):
    ...
    def compute_loss(self, **kwargs) -> dict:
        ...
        **kwargs
        ...
        if loss_type == "img_norm":
        ...
        elif loss_type == "l2_and_img_norm":
            img_hat, scaling_factor = self.forward(text_embedding)
            recon_loss = self.reconstruction_loss(img, img_hat, "l2")
            img_intermedaite = self.encoder(img)
            img_intermediate_flat = torch.flatten(img_intermedaite, start_dim=1)
            mu = self.fc_mu(img_intermediate_flat)
            ...
            norm_loss = F.mse_loss(
                mu_norm,
                torch.abs(scaling_factor).squeeze(-1),
                reduction="mean",
            )
