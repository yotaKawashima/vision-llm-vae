...
clip_grad_norm = 1.0
###### Model ######
model_type = "beta_vae_llm"  # "decoder"  # "beta_vae"  # "encoder", "ae", "beta_vae", "beta_vae_llm"
resnet_flag = False
...
# )
# checkpoint_path = Path(
#     "/mnt/data/checkpoints/decoder_loss_l2/cocoDoerig/run_0/checkpoint_epoch10.ckpt"
# )
checkpoint_path = Path(
    "/mnt/data/checkpoints/beta_vae_llm_loss_l2_and_img_norm_delta0.1/cocoDoerig/run_0/cocoDoerig/run_0/checkpoint_epoch50.ckpt"
)
...
MODEL_CONFIGS = {
    ...
    "beta_vae": {
        "loss_type": "standard",  # "standard",  # or "llm_alignment"
...
# evaluation paths (only used if eval_flag is True)
if eval_flag:
    if checkpoint_path is not None:
        if model_type == "beta_vae" and ae_checkpoint and not vae_checkpoint:
            # when evaluating an initialized vae model, checkpoint is not vae checkpoint but ae checkpoint, so we need to set the checkpoint path stem and parent accordingly
            checkpoint_path_stem = "checkpoint_epoch0"
            checkpoint_path_parent = coco_checkpoints_dir_path
        else:
    ...
    evaluation_data_dir_path = (
        checkpoint_path_parent / "evaluation" / checkpoint_path_stem
    )
